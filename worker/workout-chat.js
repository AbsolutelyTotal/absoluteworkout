...
function buildMessages({ question, context, rules, history = [] }) {
  const system = [
    ...
    'replaced. Choose it from the "permittedLibrary" list in the data — never',
    'invent one, and never suggest something that trains a different muscle just',
    'because it is on the same day. If nothing in the library matches the muscle,',
    'say so rather than offering an unrelated movement.',
    '',
